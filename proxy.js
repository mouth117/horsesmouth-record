...
export default async function handler(req, res) {
  ...
  // Strip /api/proxy prefix, then add /api back
  // e.g. /api/proxy/upload -> /api/upload
  const url = req.url || '';
  const strippedPath = url.replace(/^\/api\/proxy/, '/api');
  const targetUrl = TARGET + strippedPath;
